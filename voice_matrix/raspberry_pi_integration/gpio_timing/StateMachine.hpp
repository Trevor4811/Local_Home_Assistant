...
class Listening : public StateMachine
{
  void entry(void) override {
    ...
    //TODO gpio high
    // Serial.println("--------------- IO high");
    digitalWrite(LATENCY_IO, HIGH);
...
class MQTTDisconnected : public StateMachine {
  ...
  void entry(void) override {
    ...
    // => indicate this
    pinMode(LATENCY_IO, OUTPUT);   // active low
    digitalWrite(LATENCY_IO, LOW);
...
void onMqttMessage(char *topic, char *payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total)
{
  ...
  if (len + index == total)
  {
    ...
    {
      //TODO gpio low, return message completed
      // Serial.println("--------------- IO low");
      digitalWrite(LATENCY_IO, LOW);
